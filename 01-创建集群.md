...
AWS的Github上官方的文档使用的是部署在Github上的配置文件，在国内网络条件下访问可能会有问题。因此本实验按照中国区可以直接操作的提供了对应的配置文件。执行如下命令。
...
测试完成后，如果需要删除Dashboard，执行如下命令。本命令为可选，不需要删除可继续后续实验。
...
这个测试应用将在当前集群的两台Control node上创建nginx应用，然后通过NLB对外发布在80端口。
